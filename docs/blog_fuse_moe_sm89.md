...
数据与模型吻合得很干净：avg=32 时强制 128 比 32 慢 **51%**（padding 空转 + task 太少）；avg=4096 时强制 32 比最优慢 **95%**（W 面板装载次数 ×4）；E=64 的 W-bound 形状上即使 avg=32，128 也反超（W 流量主导一切）。一个诚实的脚注：avg=256 边界附近 64 与 128 在 ±7% 内互有胜负（S=1024/E=8 时 64 略优，H=4096 时 128 略优——W footprint 越大越偏 128），当前阈值 `avg≥256 → 128` 是在 sm120 上定标后沿用的，按架构/形状微调还能挤出几个点。
...
- **kTileM 阈值按 sm120 定标**：avg=256 附近在 sm89 上 64/128 互有胜负（§3.4），per-arch 定标还有几个点；
